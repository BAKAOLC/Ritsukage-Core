...
namespace Ritsukage.Library.Subscribe.Listener
{
    public class BilibiliLiveListener : Base.SubscribeListener
    {
        ...
        private readonly List<BilibiliLiveCheckMethod> Checker = new();
        ...
        public override async void Broadcast(CheckResult.Base.SubscribeCheckResult result)
        {
            if (result.Updated && result is BilibiliLiveCheckResult b)
            {
                ...
                var records =
                    await Database.GetArrayAsync<SubscribeList>(x => x.Type == type && x.Target == b.RoomId.ToString());
        ...
        private static string GetLiveStatus(LiveStatus status)
        {
        ...
        private static object[] GetQQMessageChain(BilibiliLiveCheckResult result)
        {
            ArrayList msgs = new();
            if (result.UpdateType == BilibiliLiveUpdateType.Initialization)
            {
                var img = DownloadManager.Download(result.Cover, enableAria2Download: true, enableSimpleDownload: true)
                    .Result;
            ...
            else if (result.UpdateType == BilibiliLiveUpdateType.LiveStatus)
            {
                ...
                {
                    var img = DownloadManager
                        .Download(result.Cover, enableAria2Download: true, enableSimpleDownload: true).Result;
                    msgs.Add(string.IsNullOrEmpty(img) ? "[图像下载失败]" : SoraSegment.Image(img));
                    msgs.Add(new StringBuilder()
                        .AppendLine()
        ...
        private static string GetDiscordMessageChain(BilibiliLiveCheckResult result)
        {
            if (result.UpdateType == BilibiliLiveUpdateType.Initialization)
            {
