...
{
    internal partial class Bilibili
    {
        ...
        public static string NewLoginRequest()
        {
            return Utils.HttpGET(GetLoginUrl);
        }

        public static string GetLoginInfo(string oauthKey)
        {
            return Utils.HttpPOST(GetLoginInfoUrl, $"oauthKey={oauthKey}&gourl=https%3A%2F%2Fwww.bilibili.com%2F");
        }
        ...
        {
            return Utils.HttpPOST(PostDynamicUrl,
                $"dynamic_id=0&type=4&rid=0&content={Utils.UrlEncode(msg)}&extension=%7B%22emoji_type%22%3A1%7D&at_uids=&ctrl=%5B%5D&csrf_token={GetJCT(cookie)}",
                20000, cookie, "https://t.bilibili.com/", "https://t.bilibili.com");
        }

        public static string GetJCT(string cookie = "")
        {
            return GetJCTMatchRegex().Match(GetEmptyCharRegex().Replace(cookie, "")).Value;
        }

        public static void QRCodeLoginRequest(Action<Image<Rgba32>> GetQRCode, Action IsScanned,
            Action<string> LoginSuccess, Action<string> LoginFailed)
        {
            Task.Run(() =>
            {
