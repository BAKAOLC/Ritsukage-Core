...
namespace Ritsukage.Library.Bilibili.Model
{
    public class Dynamic
    {
        ...
        public int View;
        public int Repost;
        public int Reply => Card == null ? 0 : Card.Reply;
        public int Like;
        ...
        public string Url => "https://t.bilibili.com/" + Id;
        ...
        {
            return OriginalCard == null ? null : Get(OriginalCard.Id);
        }
        ...
        public async Task<Image<Rgba32>> GetNinePicture()
        {
            return await Task.Run(async () =>
            {
                try
                {
                    ...
                    var pics = await DownloadManager.Download(Pictures, enableAria2Download: true,
                        enableSimpleDownload: true);
                    foreach (var file in pics)
                    {
                        if (string.IsNullOrEmpty(file)) return null;
                        var img = LoadImage(file, out var format);
        ...
        private static void DrawImage(ref Image<Rgba32> image, int dx, int dy, Image<Rgba32> draw)
        {
        ...
        {
            var data = JObject.Parse(Utils.HttpGET(
        ...
        public static Dynamic GetByJson(JToken data)
        {
            ...
            try
            {
                switch ((int)data["desc"]["type"])
                {
                    ...
                    case 4: //纯文本
                    {
                        ...
                        {
                            dynamic.Card.Content = new() { Text = (string)card["item"]["content"] };
                        }
                        else
                        {
                            dynamic.Card.Content = new() { Text = (string)card["item"]["description"] };
                        }
                    ...
                    case 8: //视频
                    {
                        ...
                        dynamic.Card.Content = new() { Text = (string)card["dynamic"] };
                    ...
                    case 256: //音频
                    {
                        ...
                        dynamic.Card.Content = new() { Text = (string)card["intro"] };
                    ...
                    case 2048: //分享
                    {
                        ...
                        dynamic.Card.Content = new() { Text = (string)card["vest"]["content"] };
            ...
            {
                throw new($"Error in parsing dynamic(ID: {dynamic.Id})", ex);
            }
                    ...
                    switch ((int)data["desc"]["orig_type"])
                    {
                        ...
                        case 4: //纯文本
                        {
                            ...
                            if (original["item"]["pictures"] != null)
                            {
                                ...
                                dynamic.OriginalCard.Content = new()
                                {
                            ...
                            else
                            {
                                ...
                                if (original["item"]["content"] != null)
                                    dynamic.OriginalCard.Content = new() { Text = (string)original["item"]["content"] };
                                else
                                    dynamic.OriginalCard.Content = new()
                                        { Text = (string)original["item"]["description"] };
                            }
                        ...
                        case 8: //视频
                        {
                            ...
                            dynamic.OriginalCard.Content = new() { Text = (string)original["dynamic"] };
                        ...
                        case 256: //音频
                        {
                            ...
                            dynamic.OriginalCard.Content = new() { Text = (string)original["intro"] };
                        ...
                        case 2048: //分享
                        {
                            ...
                            dynamic.OriginalCard.Content = new() { Text = (string)original["vest"]["content"] };
                            dynamic.OriginalCard.Sketch = DynamicSketch.GetByJson(original["sketch"]);
                            break;
                        }
                    }

                    if (data["display"] != null && data["display"]["origin"] != null &&
                        data["display"]["origin"]["add_on_card_info"] != null) //额外信息
                        dynamic.OriginalCard.Extend =
                            DynamicDisplayExtendInfo.GetByJson(data["display"]["origin"]["add_on_card_info"][0]);
    ...
    public class DynamicDisplayExtendInfo
    {
        ...
        private DynamicDisplayExtendInfo()
        {
        }
        ...
        public override string ToString()
        {
            ...
            switch (Type)
            {
                ...
                {
                    var f1 = !string.IsNullOrWhiteSpace(DescFirst);
                    var f2 = !string.IsNullOrWhiteSpace(DescSecond);
                    if (f1 || f2)
                        sb.AppendLine();
                    if (f1 && f2)
    ...
    public class DynamicLottery
    {
        ...
        public static DynamicLottery GetByDynamicId(ulong id)
        {
            var info = JObject.Parse(
                Utils.HttpGET("https://api.vc.bilibili.com/lottery_svr/v1/lottery_svr/lottery_notice?dynamic_id=" +
                              id));
            if ((int)info["code"] != 0)
    ...
    public static class DynamicExtensions
    {
        public static string Like(ulong id, bool like, string cookie)
        {
            ...
            if (int.TryParse(cookie.Split(";").Where(x => x.StartsWith("DedeUserID=")).First().Substring(11),
                    out var uid))
            {
                var param = string.Join("&",
                    ...
                    "up=" + (like ? 1 : 2),
                    "csrf_token=" + jct);
                var data = JObject.Parse(Utils.HttpPOST(
                    "https://api.vc.bilibili.com/dynamic_like/v1/dynamic_like/thumb", param, 5000, cookie));
                if ((int)data["code"] == 0)
                    return "操作执行成功";
                else
                    throw new((string)data["message"]);
