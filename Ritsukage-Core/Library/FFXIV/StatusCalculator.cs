...
namespace Ritsukage.Library.FFXIV
{
    ...
    {
        private const int nlv = 1900;

        private static readonly string Title = $"版本 6.0  Lv90  等级基数: {nlv}";

        public struct CriticalHitResult
        {
            private const int bn = 400;

            private const int fn = 200;

            public int Value { get; set; }

            private double Critical => Math.Floor(fn * ((double)Value - bn) / nlv);
            ...
            {
                return new((int)Math.Ceiling((value * 10 - 50) * nlv / fn + bn));
            }

            public static CriticalHitResult GetFromBonus(double value)
            {
                return new((int)Math.Ceiling((value * 10 - 1400) * nlv / fn + bn));
            }
        ...
        public struct DirectHitResult
        {
            ...
            private double Direct => Math.Floor(fn * ((double)Value - bn) / nlv);
            ...
            {
                return new((int)Math.Ceiling(value * 10 * nlv / fn + bn));
            }
        ...
        {
            private const int bn = 390;

            private const double fn = 140;

            public int Value { get; set; }

            private double Determination => Math.Floor(fn * ((double)Value - bn) / nlv);
            ...
            {
                return new((int)Math.Ceiling((value * 1000 - 1000) * nlv / fn + bn));
            }
        ...
        public struct TenacityResult
        {
            ...
            private const double fn = 100;

            public int Value { get; set; }

            private double Tenacity => Math.Floor(fn * ((double)Value - bn) / nlv);
            ...
            {
                return new((int)Math.Ceiling((1000 - value * 10) * nlv / fn + bn));
            }
        ...
        public struct SpeedResult
        {
            ...
            private const double fn = 130;

            public int Value { get; set; }

            private double Speed => Math.Floor(fn * ((double)Value - bn) / nlv);

            public double Dot => (1000 + Speed) / 1000;

            private double GCD => (1000 - Speed) / 1000;
