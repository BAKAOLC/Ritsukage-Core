...
namespace Ritsukage.Discord.Commands
{
    public class Time : ModuleBase<SocketCommandContext>
    {
        ...
        {
            await ReplyAsync(DateTime.Now.ToString("yyyy年MM月dd日 HH时mm分ss秒"));
        }
        ...
        private static long GetEorzeaHour(long unix)
        {
            return unix / 175 % 24;
        }

        private static long GetEorzeaMinute(long unix)
        {
            return Convert.ToInt64(60 * ((double)unix / 175 % 1));
        }
        ...
        {
            var unix = DateTimeOffset.FromUnixTimeSeconds(Utils.GetNetworkTimeStamp()).ToUniversalTime()
                .ToUnixTimeSeconds();
