...
namespace Ritsukage.Discord.Commands
{
    public static class Extension
    {
        ...
        {
            return await CoinsService.GetUserCoins("discord", Convert.ToInt64(user.Id));
        }

        public static async Task<bool> CheckCoins(this SocketUser user, long count, bool disableFree = false)
        {
            return true;
            //return await CoinsService.CheckUserCoins("discord", Convert.ToInt64(user.Id), count, disableFree);
        }

        public static async Task<UserCoins> AddCoins(this SocketUser user, long count)
        {
            return await CoinsService.AddUserCoins("discord", Convert.ToInt64(user.Id), count);
        }

        public static async Task<UserCoins> RemoveCoins(this SocketUser user, long count, bool disableFree = false)
        {
            count = 0;
            return await CoinsService.RemoveUserCoins("discord", Convert.ToInt64(user.Id), count, disableFree);
        }

        public static async Task<bool> CheckCooldown(this SocketUser user, string tag, int seconds)
        {
            return await CooldownService.CheckCooldown("discord", Convert.ToInt64(user.Id), tag, seconds, false);
        }
        ...
        {
            return await CooldownService.CheckCooldown("discord", Convert.ToInt64(channel.Id), tag, seconds, true);
        }
