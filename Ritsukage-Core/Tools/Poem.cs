...
namespace Ritsukage.Tools
{
    ...
    {
        private static string DatabasePath => Program.Config.PoetryDatabasePath;

        private const string SearchPage = "https://sou-yun.cn/CharInClause.aspx?c=";
        ...
        public static async Task<List<string>> Search(string _char)
        {
            return await Task.Run(async () =>
            {
                ...
                else
                {
                    ...
                    var tang2 = await db.Table<Poetry_Tang_300>().Where(x => x.Paragraphs.Contains(_char))
                        .ToListAsync();
        ...
        public static async Task<string> GetOrigin(string poem)
        {
            return await Task.Run(async () =>
            {
                ...
                else
                {
                    ...
                    {
                        var count = classic.Count + tang1.Count + tang2.Count + song.Count;
                        var limit = 10;
                        List<string> result = new();
        ...
        [GeneratedRegex(
            "<li class='label'>(?<author>[^<]+)</li><li class='none'><span class='poemSentence'>(?<sentence>[^<]+)</span>&nbsp;<a target='_blank' class='small' href=\"[^\"]+\">(?<poem>[^<]+)</a></li>")]
