...
namespace Ritsukage.QQ.Events
{
    ...
    public static partial class SmartPixivLink
    {
        ...
        private static readonly Dictionary<long, Dictionary<int, DateTime>> Delay = new();

        private const string Host = "www.pixiv.net/";
        private const string IllustID = "illust_id=";
        private const string Artworks = "artworks/";

        private static void Trigger(GroupMessageEventArgs args)
        {
                ...
                try
                {
                    Commands.Pixiv.GetIllustDetail(illusts,
                        async (msg) => await args.SourceGroup.SendGroupMessage(
                            SoraMessage.BuildMessageBody(new object[] { SoraSegment.Reply(args.Message.MessageId) }
                                .Concat(msg).ToArray())),
