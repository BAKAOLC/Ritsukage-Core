...
namespace Ritsukage.QQ.Service
{
    ...
    public static class TipService
    {
        ...
        private static readonly int CheckInterval = 500;
        ...
        private static async Task<TipMessage[]> GetTipMessages()
        {
            var now = DateTime.Now;
            var messages = await TipMessageService.GetTipMessages(TipMessage.TipTargetType.QQGroup, now)
                .ConfigureAwait(false);
            await TipMessageService.RefreshTipMessages(now).ConfigureAwait(false);
            return messages;
        }

        private static MessageBody BuildMessage(TipMessage message)
        {
            var m = new ArrayList
            {
                "[Tip Message]",
            ...
            var n = 0;
            var i = message.Message.IndexOf("[@all]", n);
        ...
        private static void SendMessage(TipMessage message, params long[] bots)
        {
        ...
        private static async void CheckMethod()
        {
            var messages = await GetTipMessages().ConfigureAwait(false);
            if (messages.Any())
            {
                var bots = Program.QQServer.GetBotList();
                foreach (var message in messages) SendMessage(message, bots);
            }
        }
    }
}