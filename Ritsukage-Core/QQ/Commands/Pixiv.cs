...
namespace Ritsukage.QQ.Commands
{
    ...
    public static class Pixiv
    {
        ...
        [ParameterDescription(1, "Issust ID")]
        ...
        private static async void InnerGetIllustDetail(int id, Action<object[]> Reply, Action<object[]> SendMessage,
            bool slient = false)
        {
            try
            {
                ...
                if (detail == null)
                {
                    if (!slient) Reply?.Invoke(new object[] { $"数据(pid:{id})获取失败，请稍后再试" });
                    return;
                }
                ...
                if (detail.IsUgoira)
                {
                    ...
                    if (ugoira == null)
                    {
                        if (!slient) Reply?.Invoke(new object[] { $"动图数据(pid:{id})获取失败" });
                    }
                    else
                    {
                        if (!slient) Reply?.Invoke(new object[] { $"动图数据(pid:{id})获取成功，正在进行压缩..." });
            ...
            catch (Exception ex)
            {
                ...
                if (!slient) Reply?.Invoke(new object[] { $"处理作品(pid:{id})时发生异常错误，任务已终止" });
            }
        }

        public static void GetIllustDetail(int id, Action<object[]> Reply, Action<object[]> SendMessage,
            bool slient = false)
        {
            if (!slient)
        ...
        public static void GetIllustDetail(int[] ids, Action<object[]> Reply, Action<object[]> SendMessage,
            bool slient = false)
        {
            if (!slient)
